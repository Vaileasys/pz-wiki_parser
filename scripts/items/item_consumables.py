    ...
    if item.type == "Food" and (item.is_cookable or item.get("DaysTotallyRotten")):
        try:
            ...
            parameters = {
                ...
                "cooked_image": get_variant(item, "cooked") if item.is_cookable else None,
                "rotten_image": get_variant(item, "rotten") if perishable else None,
                "burned_image": get_variant(item, "burnt") if item.is_cookable else None,
                "hunger": util.convert_int(item.hunger_change) if item.hunger_change else None,
                "boredom": util.convert_int(item.boredom_change) if item.boredom_change else None,
                "unhappiness": util.convert_int(item.unhappy_change) if item.unhappy_change else None,
                ...
                "RemoveNegativeEffectOnCooked": "true" if item.remove_negative_effect_on_cooked else None
            }
...
def process_items(items_list):
    ...
    for item_id in items_list:
        ...
        output_dir = write_file(content, rel_path=f"{item.item_id}.txt", root_path=ROOT_DIR, suppress=True)
