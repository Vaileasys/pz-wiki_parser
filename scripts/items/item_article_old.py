...
def generate_intro(lowercase_name, language_code):
    # If lowercase_name is empty, return an empty string
    ...
    try:
        # Safely get article with error handling
        article_func = language_data.get("article", lambda _: "")
        try:
            article = article_func(lowercase_name).lower()
        except (IndexError, TypeError):
            # If article function fails, use empty string
            article = ""

        # Format the intro template
        intro = intro_template.format(article=article, lowercase_name=lowercase_name)

        # Check if intro is not empty before trying to capitalize
        if intro:
            ...
            # Capitalize first letter if found
        ...
        return "This is an item in Project Zomboid."
...
def generate_crafting(item_id, crafting_dir, teachedrecipes_dir, language_code):
    ...
    if os.path.isfile(research_fp):
        try:
            ...
            parts.append(f"====Researchable recipes====\n{research}")
        except Exception as e:
    ...
    if os.path.isfile(learned_fp):
        try:
            ...
            parts.append(f"====Learned recipes====\n{learned}")
        except Exception as e:
...
def generate_obtaining(
    item_id, crafting_dir, original_filename, infobox_name, distribution_dir
):
    parts = []
    # Recipes
...
def load_infoboxes(infobox_dir):
    ...
    for fname in os.listdir(infobox_dir):
        ...
        infobox_data.append(
            {
                "name": re.search(r"\|name\s*=\s*(.+)", txt).group(1).strip()
                if re.search(r"\|name\s*=\s*(.+)", txt)
                else "",
                "category": re.search(r"\|category\s*=\s*(.+)", txt).group(1).strip()
                if re.search(r"\|category\s*=\s*(.+)", txt)
                else "",
                "skill_type": re.search(r"\|skill_type\s*=\s*(.+)", txt)
                .group(1)
                .strip()
                if re.search(r"\|skill_type\s*=\s*(.+)", txt)
                else "",
                "tags": [t.strip() for t in re.findall(r"\|tag\d?\s*=\s*(.+)", txt)],
                "item_id": re.search(r"\|item_id\s*=\s*(.+)", txt).group(1).strip()
                if re.search(r"\|item_id\s*=\s*(.+)", txt)
                else "",
            }
        )
    return infobox_data
...
def assemble_body(
    ...
    # Crafting
    crafting_inner = generate_crafting(
        item_id, crafting_dir, teachedrecipes_dir, language_code
    )
    if crafting_inner:
        body += f"\n===Crafting===\n{crafting_inner}\n"
    ...
    cond = generate_condition(
        name, category, skill_type, infobox, fixing_dir, language_code
    )
    if cond:
        body += f"\n===Condition===\n{cond}\n"

    sections = {
        "Obtaining": generate_obtaining(
            item_id, crafting_dir, original_filename, infobox_name, distribution_dir
        ),
...
def process_files(
    ...
    if not infobox_match:
        # Skip files that don't have an Infobox item without warning
        # These might be vehicle parts or other non-item infoboxes
        return
    infobox = infobox_match.group(1)

    item_id_match = re.search(r"\|item_id\s*=\s*(.+)", infobox)
    if not item_id_match:
        # For files with vehicle parts or other special items, extract a fallback ID
        # Try to extract from filename first
        filename = os.path.basename(file_path)
        filename_without_ext = os.path.splitext(filename)[0]

        # Check if there's a module_id or part_id we can use instead
        alt_id_match = re.search(r"\|(module_id|part_id)\s*=\s*(.+)", infobox)
        if alt_id_match:
            item_id = alt_id_match.group(2).strip()
        else:
            # Use the filename as a last resort
            item_id = filename_without_ext
    else:
        item_id = item_id_match.group(1).strip()

    # Skip processing if item_id is empty or already in dictionary
    if not item_id or (not generate_all and item_id in item_id_dict):
        return

    version_match = re.search(r"\|infobox_version\s*=\s*([\d\.]+)", infobox)
    infobox_version = version_match.group(1) if version_match else ""

    # Get name, use item_id as fallback if name is missing
    name_match = re.search(r"\|name\s*=\s*(.+)", infobox)
    name = name_match.group(1).strip() if name_match else item_id

    # If name is still empty, use the filename without extension
    if not name:
        name = os.path.splitext(os.path.basename(file_path))[0]

    lowercase_name = name.lower() if name else ""
...
def main(run_directly=False):
    ...
    consumables_dir = os.path.join("output", language_code, "consumable_properties")
    fixing_dir = os.path.join("output", language_code, "fixing")
    dictionary_dir = os.path.join("resources", "item_id_dictionary.csv")
    distribution_dir = os.path.join("output", "en", "item", "distributions")
    history_dir = os.path.join("resources", "history")
    code_dir = os.path.join("output", "en", "item", "codesnips")
    ...
    body_parts_dir = os.path.join("output", language_code, "item", "body_part")
