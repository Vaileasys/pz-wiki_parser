...
def process_item(item: Item, cut: str = None):
    ...
    item_dict["animal"] = ('<br>'.join([breed.wiki_link for breed in breeds]) if breeds else "-") if "animal" in columns else None
...
def find_table_type(item: Item):
    if item.has_tag("AnimalSkull"):
        ...
        return "bone"
    elif AnimalMeat.exists(item.item_id) or item.food_type in ("Poultry", "Game") or item.has_tag("Vermin"):
        return "meat"
...
def find_items() -> dict:
    ...
    # Process items
    with tqdm(total=len(animal_parts), desc="Processing items", bar_format=PBAR_FORMAT, unit=" items", leave=False) as pbar:
