...
TABLE_PATH = os.path.join(TABLES_DIR, "junk_table.json")
...
def find_original_item(item: Item):
    parent_item = Item(item.item_id.replace("_Broken", ""))
    if parent_item.valid and item != parent_item:
...
def main():
    ...
    table_helper.create_tables("junk", items, table_map=table_map, columns=column_headings, suppress=True, bot_flag_type="junk_item_list", combine_tables=False)
