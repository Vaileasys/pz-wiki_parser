...
def generate_data(item: Item):
    ...
    if (
        "dismantle" in columns
    ):  # FIXME: currently returns 1 product. Needs to return correct mapped product. May need work on CraftRecipe
    ...
    item_dict["degradation"] = (
        util.convert_percentage(1 / item.condition_lower_chance_one_in, decimals=1)
...
def find_table_type(item: Item):
    ...
    if (
        ...
        return "appliance"
    if (
        item.item_id in CraftRecipe("DismantleMiscElectronics").output_items
        ...
        or item.item_id
        in ("Base.RadioReceiver", "Base.RadioTransmitter", "Base.ScannerModule")
    ):
        return "component"
    if (
        item.item_id in CraftRecipe("MakeRemoteTrigger").output_items
...
def main():
    ...
    table_helper.create_tables(
        ...
        suppress=True,
        bot_flag_type="electronic_item_list",
