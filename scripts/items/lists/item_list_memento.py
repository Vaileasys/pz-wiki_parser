...
def generate_data(item: Item):
    ...
    if "capacity" in columns:
        if item.fluid_container:
            item_dict["capacity"] = str(util.convert_int(item.fluid_container.capacity)) + " L"
...
def main():
    ...
    table_helper.create_tables("memento_item_list", items, table_map=table_map, columns=column_headings, suppress=True, bot_flag_type="memento_item_list", combine_tables=False)
