...
TABLE_PATH = os.path.join(TABLES_DIR, "entertainment_table.json")
...
def process_items() -> dict:
    ...
    with tqdm(total=Item.count(), desc="Processing items", bar_format=PBAR_FORMAT, unit=" items", leave=False) as pbar:
        for item_id, item in Item.items():
            ...
            if item.has_category("entertainment"):
...
def main():
    ...
    table_helper.create_tables("entertainment_item_list", items, table_map=table_map, columns=column_headings, suppress=True, bot_flag_type="entertainment_item_list", combine_tables=False)
