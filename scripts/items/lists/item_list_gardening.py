...
def generate_data(item: Item):
    ...
    item_dict["sprite"] = (
        crop.get_sprite(dim="64x64px") if "sprite" in columns else None
    )
    ...
    item_dict["grow_time"] = (
        f"{crop.grow_time_days} days" if "grow_time" in columns else None
    )
    item_dict["sow_months"] = (
        "<br>".join(crop.sow_months) if "sow_months" in columns else None
    )
    item_dict["best_months"] = (
        ("<br>".join(crop.best_months) if crop.best_months else "-")
        if "best_months" in columns
        else None
    )
    item_dict["poor_months"] = (
        ("<br>".join(crop.risk_months) if crop.risk_months else "-")
        if "poor_months" in columns
        else None
    )
    item_dict["teached_recipes"] = (
        "<br>".join(get_teached_recipes(item)) if "teached_recipes" in columns else None
    )
    ...
    if "dirt" in columns:
        if item.has_tag("TakeDirt"):
            item_dict["dirt"] = util.tick(
                text="Can pick-up dirt", link="TakeDirt (tag)"
            )
        else:
            item_dict["dirt"] = util.cross(
                text="Cannot pick-up dirt", link="TakeDirt (tag)"
            )
    if "dung" in columns:
        if item.has_tag("TakeDung"):
            item_dict["dung"] = util.tick(
                text="Can pick-up dung", link="TakeDung (tag)"
            )
        else:
            item_dict["dung"] = util.cross(
                text="Cannot pick-up dung", link="TakeDung (tag)"
            )
    if "plow" in columns:
    ...
    if "scythe" in columns:
        if item.has_tag("Scythe"):
            item_dict["scythe"] = util.tick(
                text="Can scythe grass", link="Scythe (tag)"
            )
        else:
            item_dict["scythe"] = util.cross(
                text="Cannot scythe grass", link="Scythe (tag)"
            )
    if "cut_plant" in columns:
        if item.has_tag("CutPlant"):
            item_dict["cut_plant"] = util.tick(
                text="Can cut plants", link="CutPlant (tag)"
            )
        else:
            item_dict["cut_plant"] = util.cross(
                text="Cannot cut plants", link="CutPlant (tag)"
            )
...
def main():
    ...
    table_helper.create_tables(
        ...
        suppress=True,
        bot_flag_type="gardening_item_list",
