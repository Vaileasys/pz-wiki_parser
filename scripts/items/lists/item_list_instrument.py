...
TABLE_PATH = os.path.join(TABLES_DIR, "instrument_table.json")
...
def generate_data(item: Item):
    ...
    if "play_sound" in columns:
        if item.shout_type:
            item_dict["play_sound"] = util.split_camel_case((util.link("Noise", item.shout_type)))
        else:
            item_dict["play_sound"] = "-"
...
def process_items() -> dict:
    ...
    with tqdm(total=Item.count(), desc="Processing items", bar_format=PBAR_FORMAT, unit=" items", leave=False) as pbar:
        for item_id, item in Item.items():
            ...
            if item.has_category("instrument"):
...
def main():
    ...
    table_helper.create_tables("instrument_item_list", items, table_map=table_map, columns=column_headings, suppress=True, bot_flag_type="instrument_item_list", combine_tables=False)
