...
def load_item_dictionary(item_name):
    ...
    for key, value in item_dict.items():
        if formatted_item_name == f'"{value.replace(" ", "")}"':
            ...
            item_name_changes[item_name] = (
                new_item_id  # Store original and new item name
            )
            return new_item_id
...
def process_json(file_paths):
    ...
    for file_key, fp in file_paths.items():
        ...
        if file_key == "proceduraldistributions":
            ...
            for fish_item_id, fish_data in data.items():
                ...
                if not (
                    "lure" in fish_data
                    or "isRiver" in fish_data
                    or "isLake" in fish_data
                ):
                    continue
                # Remove "Base." prefix if present to match other item processing
                item_name = (
                    fish_item_id.replace("Base.", "")
                    if fish_item_id.startswith("Base.")
                    else fish_item_id
                )
            ...
            for animal_name, animal_data in animals_data.items():
                ...
                if "head" in animal_data:
                    ...
                    head_item = (
                        head_item.replace("Base.", "")
                        if head_item.startswith("Base.")
                        else head_item
                    )
                ...
                if "skull" in animal_data:
                    ...
                    skull_item = (
                        skull_item.replace("Base.", "")
                        if skull_item.startswith("Base.")
                        else skull_item
                    )
                ...
                if "feather" in animal_data:
                    ...
                    feather_item = (
                        feather_item.replace("Base.", "")
                        if feather_item.startswith("Base.")
                        else feather_item
                    )
                ...
                if "leather" in animal_data:
                    ...
                    leather_item = (
                        leather_item.replace("Base.", "")
                        if leather_item.startswith("Base.")
                        else leather_item
                    )
                ...
                if "parts" in animal_data:
                    for part in animal_data["parts"]:
                        ...
                        part_item = (
                            part_item.replace("Base.", "")
                            if part_item.startswith("Base.")
                            else part_item
                        )
                ...
                if "bones" in animal_data:
                    for bone in animal_data["bones"]:
                        ...
                        bone_item = (
                            bone_item.replace("Base.", "")
                            if bone_item.startswith("Base.")
                            else bone_item
                        )
                        item_list.add(bone_item)
                        count += 1

        elif file_key == "container_contents":
            # Process container contents data
            if not data:
                continue
            ...
            for container_id, container_data in data.items():
                ...
                for item_name in items.keys():
                    ...
                    clean_item_name = (
                        item_name.replace("Base.", "")
                        if item_name.startswith("Base.")
                        else item_name
                    )
                    item_list.add(clean_item_name)
                    count += 1
    ...
    with open(
        os.path.join("output", "en", "item", "distributions", "Item_list.txt"), "w"
    ) as output_file:
...
def build_item_json(
    ...
    def get_container_info(item_name):
        ...
        # Add container contents data
        container_contents_matches = get_container_contents_info(
            item_name, container_contents_data
        )
        for match in container_contents_matches:
            # For container contents, we use the chance directly (not rolls-based calculation)
            entry_tuple = (
                match["Room"],
                match["Container"],
                match["Chance"],
    ...
    def get_vehicle_info(item_name):
        ...
        special_vehicles = [
            ...
            "PackRat",
            "StepVan_Plonkies",
            "StepVan_AirportCatering",
            "VanSeats_AirportShuttle",
            "StepVan_MarineBites",
            "StepVan_Zippee",
            "StepVan_Soda",
            "StepVan_Beer",
            ...
            # If no suffix found, check for container types at the beginning
                ...
                if not prefix_match:
                    ...
                    if not special_vehicle_match:
                        ...
                        is_base_entry = any(
                            entry.startswith(label) and entry != label
                            for entry in vehicle_data.keys()
                        )
                        if is_base_entry:
                            vehicle_type = label
                            container = "Base"  # Marking as base entry
    ...
    def get_butchering_info(item_name):
        ...
        animals_data = butchering_data.get("AnimalPartsDefinitions", {}).get(
            "animals", {}
        )
        ...
        for animal_name, animal_data in animals_data.items():
            ...
            if "head" in animal_data:
                ...
                if head_item_clean == item_name:
                    butchering_matches.append(
                        {"animal": animal_name, "amount": "1", "type": "head"}
                    )
            ...
            if "skull" in animal_data:
                ...
                if skull_item_clean == item_name:
                    butchering_matches.append(
                        {"animal": animal_name, "amount": "1", "type": "skull"}
                    )
            ...
            if "feather" in animal_data:
                ...
                if feather_item_clean == item_name:
                    butchering_matches.append(
                        {"animal": animal_name, "amount": "1", "type": "feather"}
                    )
            ...
            if "leather" in animal_data:
                ...
                if leather_item_clean == item_name:
                    butchering_matches.append(
                        {"animal": animal_name, "amount": "1", "type": "leather"}
                    )
            ...
            if "parts" in animal_data:
                for part in animal_data["parts"]:
                    ...
                    if part_item_clean == item_name:
                        ...
                        butchering_matches.append(
                            {"animal": animal_name, "amount": amount, "type": "parts"}
                        )
            ...
            if "bones" in animal_data:
                for bone in animal_data["bones"]:
                    ...
                    if bone_item_clean == item_name:
                        ...
                        butchering_matches.append(
                            {"animal": animal_name, "amount": amount, "type": "bones"}
                        )

        return butchering_matches

    def get_container_contents_info(item_name, container_contents_data):
        """
        Get container contents information for an item.

        This function checks all container contents for the specified item and returns
        information about which containers can contain the item with their chances.

        Args:
            item_name (str): The item to search for
            container_contents_data (dict): Container contents data from cache
        ...
        if not container_contents_data:
            return container_matches

        for container_id, container_data in container_contents_data.items():
            if not isinstance(container_data, dict):
                continue
            items = container_data.get("items", {})

            # Check if our item is in this container's items
            if item_name in items:
                chance = items[item_name]

                # Remove "Base." prefix from container_id if present
                container_name = (
                    container_id.replace("Base.", "")
                    if container_id.startswith("Base.")
                    else container_id
                )
                ...
                container_matches.append(
                    {
                        ...
                        "Container": container_name,
                        "Chance": chance,
                        "Rolls": 1,  # Container contents use direct probability, not rolls-based
                    }
                )
    ...
    def get_attached_weapon_info(item_name, attached_weapons_data, outfits_data=None):
        ...
        for def_name, def_data in global_definitions.items():
            ...
            if any(
                weapon == full_item_name or weapon == item_name_no_prefix
        ...
        if global_applicable_defs:
            ...
            truly_global_defs = [
                d for d in global_applicable_defs if not d["outfit_filter"]
            ]
            ...
            if truly_global_defs:
                ...
                for definition in truly_global_defs:
                    definition_selection_weight = (
                        definition["chance"] / total_global_weight
                        if total_global_weight > 0
                        else 0
                    )
                    item_selection_weight = 1.0 / len(definition["weapons"])
                    effective_chance_percentage = (
                        global_base_chance
                        * definition_selection_weight
                        * item_selection_weight
                        * 100
                    )
        ...
        for outfit_name, outfit_config in outfit_definitions_data.items():
            ...
            ):  # If no time period in name, use original value
            ...
            for weapon_def in outfit_weapons:
                if isinstance(weapon_def, dict) and "weapons" in weapon_def:
                    ...
                    if any(
                        ...
                        weapon_days = get_days_from_outfit_name(
                            weapon_def.get("id", "")
                        )
                        if (
                            weapon_days == 0
            ...
            if outfit_applicable_defs:
                ...
                for definition in outfit_applicable_defs:
                    definition_selection_weight = (
                        definition["chance"] / total_outfit_weight
                        if total_outfit_weight > 0
                        else 0
                    )
                    item_selection_weight = 1.0 / len(definition["weapons"])
                    effective_chance_percentage = (
                    ...
                    attached_weapon_matches.append(
                        {
                            ...
                            "days_required": definition[
                                "days_required"
                            ],  # Using the higher value we stored earlier
                        }
                    )
        ...
        for outfit_name in all_mentioned_outfits:
            ...
            if (
                ...
                outfit_days_required = outfit_definitions_data.get(outfit_name, {}).get(
                    "daySurvived", 0
                )

            # Check global definitions that specify this outfit
            global_for_outfit = [
                d for d in global_applicable_defs if outfit_name in d["outfit_filter"]
            ]
            ...
            if global_for_outfit:
                ...
                if outfit_name in outfit_definitions_data:
                    ...
                    for definition in global_for_outfit:
                        definition_selection_weight = (
                            definition["chance"] / total_weight
                            if total_weight > 0
                            else 0
                        )
                        item_selection_weight = 1.0 / len(definition["weapons"])
...
def build_tables(category_items, index):
    ...
    def write_header(f, category, part_number=None):
        ...
        f.write(
            "foraging = {amount, level, snow, rain, day, night, biome_table, months_table, bonus_table, malus_table}\n"
        )
        f.write("fishing = {lure_link, chance}\n")
        f.write("butchering = {animal_name, amount}\n")
        f.write(
            "attachedWeapons = {outfit, definition_id, days_required, effective_chance}\n"
        )
        f.write("--]]\n\n")
        f.write("local data = {\n")
        ...
        for item_id, item_data in sorted_items.items():
            ...
            if item_data.get("Containers"):
                ...
                for container in item_data["Containers"]:
                    ...
                    # For container contents (Room="Any"), use the chance directly
                    # For other containers, calculate effective chance using rolls
                    if room == "Any":
                ...
                if container_data:
                    ...
                    container_lines = [
                        f'{{"{room}", "{container_name}", {effective_chance}}}'
            ...
            if item_data.get("Vehicles"):
                ...
                if vehicle_data:
                    ...
                    vehicle_lines = [
                        f'{{"{type_}", "{container}", {effective_chance}}}'
            ...
            if item_data.get("Stories"):
                ...
                stories = sorted(
                    item_data["Stories"], key=lambda x: (x["id"], x["link"])
                )
            ...
            if item_data.get("Foraging") and item_data["Foraging"]:
                ...
                if months:
                    month_items = [
                        f'["{month}"] = {value}' for month, value in months.items()
                    ]
                ...
                if bonus_months:
                    bonus_items = [
                        f'["{month}"] = {value}'
                        for month, value in bonus_months.items()
                    ]
                ...
                if malus_months:
                    malus_items = [
                        f'["{month}"] = {value}'
                        for month, value in malus_months.items()
                    ]
            ...
            if item_data.get("Butchering"):
                ...
                if butchering_data:
                    ...
                    butchering_lines = [
                        f'{{"{animal_name}", "{amount}"}}'
                        for animal_name, amount in butchering_data
                    ]
                    sections.append(
                        f"    butchering = {{{','.join(butchering_lines)}}}"
                    )
            ...
            if item_data.get("AttachedWeapons"):
                ...
                if attached_weapons_data:
                    ...
                    attached_weapons_data.sort(
                        key=lambda x: (x["outfit"], x["definition_id"])
                    )
                    ...
                    sections.append(
                        f"    attachedWeapons = {{{','.join(attached_weapons_lines)}}}"
                    )
    ...
    for category in tqdm.tqdm(sorted_categories, desc="Writing Lua files"):
        ...
        for item_id, item_data in sorted(items.items()):
            ...
            if estimated_lines + item_lines > 3250 and current_items:
                # Write current file
                file_suffix = (
                    "" if current_file_number == 1 else str(current_file_number)
                )
                category_name = (
                    category
                    if current_file_number == 1
                    else f"{category}{current_file_number}"
                )
                output_file = os.path.join(
                    output_dir, f"{category}{file_suffix}_data.lua"
                )
                ...
                with open(output_file, "w") as f:
                    write_header(
                        f,
                        category,
                        current_file_number if current_file_number > 1 else None,
                    )
    ...
    with open(index_file, "w") as f:
        ...
        f.write(
            "For categories with multiple files, the category name will have a number suffix\n"
        )
        f.write("--]]\n\n")
...
def combine_items_by_page(all_items):
    ...
    for page_name, page_data in flattened_page_dict.items():
        ...
        primary_id = (
            primary_id_full.replace("Base.", "")
            if primary_id_full.startswith("Base.")
            else primary_id_full
        )
        ...
        for item_id_full in item_ids:
            ...
            item_id = (
                item_id_full.replace("Base.", "")
                if item_id_full.startswith("Base.")
                else item_id_full
            )
    ...
    for item_id, item_data in all_items.items():
        ...
        if is_primary:
            ...
            if secondary_data.get("Containers"):
                ...
                combined_items[primary_id]["Containers"].extend(
                    secondary_data["Containers"]
                )
            ...
            if secondary_data.get("Vehicles"):
                ...
                combined_items[primary_id]["Vehicles"].extend(
                    secondary_data["Vehicles"]
                )
            ...
            if secondary_data.get("Foraging") and secondary_data["Foraging"]:
                if (
                    not combined_items[primary_id].get("Foraging")
                    or not combined_items[primary_id]["Foraging"]
                ):
                    combined_items[primary_id]["Foraging"] = secondary_data["Foraging"]
                elif len(secondary_data["Foraging"]) > len(
                    combined_items[primary_id]["Foraging"]
                ):
                    combined_items[primary_id]["Foraging"] = secondary_data["Foraging"]
            ...
            if secondary_data.get("Clothing"):
                ...
                combined_items[primary_id]["Clothing"].extend(
                    secondary_data["Clothing"]
                )
            ...
            if secondary_data.get("Fishing") and secondary_data["Fishing"]:
                ...
                if not combined_items[primary_id]["Fishing"] or (
                    secondary_data["Fishing"].get("lures")
                    and len(secondary_data["Fishing"].get("lures", []))
            ...
            if secondary_data.get("Butchering"):
                ...
                combined_items[primary_id]["Butchering"].extend(
                    secondary_data["Butchering"]
                )
            ...
            if secondary_data.get("AttachedWeapons"):
                ...
                combined_items[primary_id]["AttachedWeapons"].extend(
                    secondary_data["AttachedWeapons"]
                )
    ...
    for item_id, item_data in combined_items.items():
        ...
        if item_data.get("Stories"):
            ...
            for story in item_data["Stories"]:
                story_key = (story.get("id", ""), story.get("link", ""))
            ...
            if item_data.get("Fishing") and item_data["Fishing"].get("lures"):
                ...
                for lure in item_data["Fishing"]["lures"]:
                    lure_key = (lure.get("name", ""), lure.get("chance", 0))
...
def main():
    ...
    for item_id, item_data in tqdm.tqdm(
        combined_items.items(), desc="Categorizing items"
    ):
        ...
        has_data = (
            (item_data.get("Containers") and len(item_data["Containers"]) > 0)
            or (item_data.get("Vehicles") and len(item_data["Vehicles"]) > 0)
            or (item_data.get("Stories") and len(item_data["Stories"]) > 0)
            or (item_data.get("Clothing") and len(item_data["Clothing"]) > 0)
            or (item_data.get("Foraging") and item_data["Foraging"])
            or (item_data.get("Fishing") and item_data["Fishing"])
            or (item_data.get("Butchering") and len(item_data["Butchering"]) > 0)
            or (
                item_data.get("AttachedWeapons")
                and len(item_data["AttachedWeapons"]) > 0
            )
        )
    ...
    missing_items_path = os.path.join(
        "output", "en", "item", "distributions", "missing_items.txt"
    )
