...
def process_location_literature(item: Item, on_create):
    ...
    businesses = {
        key: literature_data["MiscDetails"].get(key, None) for key in business_list
    }
...
def process_comic(item: Item, on_create):
    ...
    for title in comic_titles:
        nested_data = literature_data["SpecialLootSpawns"]["ComicBookDetails"].get(
            title, {"issues": "1", "inPrint": False}
        )
...
def process_magazine(item: Item, on_create):
    ...
    if on_create in ["OnCreateMagazine3"]:
        ...
        for subject in subjects:
            magazine_titles[subject] = literature_data["SpecialLootSpawns"][
                "MagazineSubjects"
            ][subject]
        ...
        subject = MAGAZINE[on_create]
        magazine_titles["Default"] = literature_data["SpecialLootSpawns"][
            "MagazineSubjects"
        ][subject]
    ...
    for key, values in magazine_titles.items():
        updated_values = {}
        for title in values:
            nested_data = literature_data["SpecialLootSpawns"]["MagazineDetails"].get(
                title, {"firstYear": "1970"}
            )
...
def process_book(item: Item, on_create):
    ...
    try:
        if on_create in on_create_books:
            ...
            for subject in subjects:
                book_titles[subject] = literature_data["SpecialLootSpawns"][
                    "BookTitles"
                ][subject]

        else:
            subject = BOOK[on_create]
            book_titles["Default"] = literature_data["SpecialLootSpawns"]["BookTitles"][
                subject
            ]
    except KeyError as e:
        from scripts.utils import echo

        echo.error(f"KeyError in process_book for {item.item_id} ({on_create}): {e}")
        echo.error("Skipping book title processing for this item.")
        return
    ...
    for subject, titles in book_titles.items():
        ...
        for title in titles:
            ...
            if (
                cover == "both"
                or (cover == "hardcover" and hardcover)
                or (cover == "softcover" and paperback)
            ):
    ...
    with open(output_path, "w", encoding="utf-8") as file:
        ...
        if literature_type == "book":
            ...
            if len(literature_titles) == 1 and "Default" in literature_titles:
                # No subjects
                file.write(
                    '{| class="wikitable theme-red mw-collapsible sortable" style="text-align: center;"\n'
                )
                file.write('|+ style="min-width: 300px;" | List of magazine variants\n')
                file.write("! Title !! Earliest year\n")
                for title in sorted(literature_titles["Default"]):
                    first_year = literature_titles["Default"][title].get(
                        "firstYear", "1970"
                    )
            ...
            file.write(
                '{| class="wikitable theme-red mw-collapsible mw-collapsed sortable" style="text-align: center;"\n'
            )
        ...
        # postcards, doodles and photos are generated the same
            ...
            file.write(
                '<div class="list-columns" style="column-width:300px; max-width:1300px;">\n'
            )
            for title in sorted(literature_titles):
            ...
            file.write(
                '{| class="wikitable theme-red mw-collapsible mw-collapsed sortable"\n'
            )
            file.write(
                f'|+ style="min-width: 300px;" | List of {item_name.lower()} titles\n'
            )
