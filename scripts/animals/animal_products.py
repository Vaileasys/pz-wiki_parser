...
def generate_data(breed: AnimalBreed) -> dict[str, str]:
    ...
    if breed.feather_item:
        ...
        products = {
            "ground": feather_str,
            "hook": feather_str,
            "ground_roadkill": feather_str,
            "hook_roadkill": feather_str,
        }
...
def do_meat_table(meats: list[AnimalMeat], breed: AnimalBreed, stat: str = "hunger"):
    ...
    content.append(
        bot_flag_start + table_helper.TABLE_WRAP_BEFORE + table_helper.DEF_TABLE_HEADER
    )
    ...
    if breed.animal.can_be_hooked:
        ...
        content.extend(["! Icon", "! Name", "! Normal", "! Roadkill"])
    ...
    content.append(
        table_helper.DEF_TABLE_FOOTER + table_helper.TABLE_WRAP_AFTER + bot_flag_end
    )

    write_file(
        content, rel_path=breed.full_breed_id + ".txt", root_path=root_path_meat + stat
    )


def get_meat_variants(
    meat: AnimalMeat, meat_ratio: list, breed: AnimalBreed
) -> list[dict]:
    variants = []
    for variant in meat.variants:
        ...
        values["hunger_hook_roadkill"] = (
            f"{hunger_roadkill[0]:.0f}–{hunger_roadkill[1]:.0f}"
        )

        # Ground
        hunger, hunger_roadkill = calculate_meat(
            base_hunger, hunger_boost, meat_ratio, ground=True
        )
        values["hunger_ground"] = f"{hunger[0]:.0f}–{hunger[1]:.0f}"
        values["hunger_ground_roadkill"] = (
            f"{hunger_roadkill[0]:.0f}–{hunger_roadkill[1]:.0f}"
        )
        ...
        # Hook
        calories, calories_roadkill = calculate_meat(
            calories_base, hunger_boost, meat_ratio
        )
        values["calories_hook"] = f"{calories[0]:.0f}–{calories[1]:.0f}"
        values["calories_hook_roadkill"] = (
            f"{calories_roadkill[0]:.0f}–{calories_roadkill[1]:.0f}"
        )

        # Ground
        calories, calories_roadkill = calculate_meat(
            calories_base, hunger_boost, meat_ratio, ground=True
        )
        values["calories_ground"] = f"{calories[0]:.0f}–{calories[1]:.0f}"
        values["calories_ground_roadkill"] = (
            f"{calories_roadkill[0]:.0f}–{calories_roadkill[1]:.0f}"
        )
        ...
        # Hook
        carbohydrates, carbohydrates_roadkill = calculate_meat(
            carbohydrates_base, hunger_boost, meat_ratio
        )
        values["carbohydrates_hook"] = f"{carbohydrates[0]:.0f}–{carbohydrates[1]:.0f}"
        values["carbohydrates_hook_roadkill"] = (
            f"{carbohydrates_roadkill[0]:.0f}–{carbohydrates_roadkill[1]:.0f}"
        )

        # Ground
        carbohydrates, carbohydrates_roadkill = calculate_meat(
            carbohydrates_base, hunger_boost, meat_ratio, ground=True
        )
        values["carbohydrates_ground"] = (
            f"{carbohydrates[0]:.0f}–{carbohydrates[1]:.0f}"
        )
        values["carbohydrates_ground_roadkill"] = (
            f"{carbohydrates_roadkill[0]:.0f}–{carbohydrates_roadkill[1]:.0f}"
        )
        ...
        # Hook
        proteins, proteins_roadkill = calculate_meat(
            proteins_base, hunger_boost, meat_ratio
        )
        values["proteins_hook"] = f"{proteins[0]:.0f}–{proteins[1]:.0f}"
        values["proteins_hook_roadkill"] = (
            f"{proteins_roadkill[0]:.0f}–{proteins_roadkill[1]:.0f}"
        )

        # Ground
        proteins, proteins_roadkill = calculate_meat(
            proteins_base, hunger_boost, meat_ratio, ground=True
        )
        values["proteins_ground"] = f"{proteins[0]:.0f}–{proteins[1]:.0f}"
        values["proteins_ground_roadkill"] = (
            f"{proteins_roadkill[0]:.0f}–{proteins_roadkill[1]:.0f}"
        )
        ...
        values["lipids_hook_roadkill"] = (
            f"{lipids_roadkill[0]:.0f}–{lipids_roadkill[1]:.0f}"
        )

        # Ground
        lipids, lipids_roadkill = calculate_meat(
            lipids_base, hunger_boost, meat_ratio, ground=True
        )
        values["lipids_ground"] = f"{lipids[0]:.0f}–{lipids[1]:.0f}"
        values["lipids_ground_roadkill"] = (
            f"{lipids_roadkill[0]:.0f}–{lipids_roadkill[1]:.0f}"
        )
...
def do_parts_table(
    results: dict[Item, dict[str, str]], breed: AnimalBreed
) -> list[str]:
    content = []
    ...
    for item, products in results.items():
        ...
        if can_be_hooked:
            content.extend(
                ["| " + products.get("hook"), "| " + products.get("hook_roadkill")]
            )
    footnote = (
        "{{Footnote|*|Obtained from an animal skeleton.|name=*}}" if is_bone else ""
    )
    content.append(
        table_helper.DEF_TABLE_FOOTER
        + footnote
        + table_helper.TABLE_WRAP_AFTER
        + bot_flag_end
    )

    write_file(
        content, rel_path=breed.full_breed_id + ".txt", root_path=root_path_parts
    )
